...
struct SidebarView: View {
    ...
    var body: some View {
        List(selection: $selectedEntry) {
                        ...
                        HStack(spacing: 8) {
                            ...
                            ForEach(store.allTags, id: \.self) { tag in
                                TagChip(label: "#\(tag)", isSelected: selectedTag == tag) {
                                    selectedTag = tag == selectedTag ? nil : tag
                                }
                            }
                        }
            ...
            ForEach(groupedEntries.keys.sorted(by: >), id: \.self) { monthKey in
                Section(header: Text(monthKey).font(.caption).foregroundStyle(.secondary)) {
                    ForEach(groupedEntries[monthKey] ?? []) { entry in
                        EntryRowView(entry: entry)
                            .tag(entry)
                    }
                    .onDelete { offsets in
                        let list = groupedEntries[monthKey] ?? []
                        store.delete(at: offsets, in: list)
                        if selectedEntry != nil && !store.entries.contains(where: { $0.id == selectedEntry?.id }) {
                            selectedEntry = nil
                        }
                    }
                }
    ...
    private var markedDates: Set<Date> {
        ...
        return Set(store.entries(for: displayMonth).map {
